...
The following code works well, because the month shortnames are 'en-US|en-GB' formatted.

````powershell
@('jan', 'feb', 'mar', 'apr', 'may', 'jun', 'jul', 'aug', 'sep', 'oct', 'nov', 'dec') | ForEach-Object {
    try{
        Get-Date "$($_)/2023" -f "MM MMM"
    }catch{
...
The following code runs in an error, because the words 'mar|märz', 'okt|okt.' or 'dez|dez.' are not valid DateTime formats for the current culture 'en-US|en-GB'.
